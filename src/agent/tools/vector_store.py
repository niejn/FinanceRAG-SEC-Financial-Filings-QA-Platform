...
def delete_document_nodes(document_id: int) -> None:
    """Delete every Qdrant point belonging to one document.

    Qdrant has no relational foreign-key/cascade semantics here: ``parent_id``
    is only payload metadata.  Deleting by ``document_id`` removes section and
    level-0 chunk points in one filtered operation; PostgreSQL remains the
    authoritative store for the actual parent-child constraint.
    """
    client = get_client()
...
def upsert_nodes(nodes: list[dict[str, Any]]) -> None:
    """Write application-generated node IDs, vectors, and filter payloads.

    ``PointStruct`` only builds an in-memory SDK object.  ``client.upsert`` is
    the call that sends it to Qdrant.  ``node_id`` is reused for the Qdrant
    point ID and PostgreSQL ``rag_nodes.id``; it is not generated by Qdrant.
    """
    if not nodes:
        return
